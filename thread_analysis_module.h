...
class thread_analysis_module{
	public:
		...
		// 	cout<<"Output header generated, start analyzing variant data."<<endl;
		// 	row_cur=0,var_num=0;
		// 	while(getline(input,line)){
		// 		if(line[0]!='#'){
		// 			if(line_cter%10000==0){
		...
		// 			//output data
		// 			// cout<<ind_vec[0]<<'\t'<<ind_vec[1]<<'\t'<<ind_vec[2]<<'\t'<<ind_vec[3]<<'\t'<<ind_vec[4]<<'\t'<<ind_vec[5]<<'\t'<<ind_vec[6]<<endl;
		// 			if(param.dist_mode){
		// 				ul.lock();
			...
			while(true){
				...
				if(sp_q.size()==0){
					...
					if(param.analysis_mode==5){
						if(!param.no_splicing){
							str_vec_pair=discover_unique_variant_1pop(pop,param,var,ann,line_vec,pop_vec,row_mark,ind_vec);
						}else{
							if (ann.gene_transcript_dict[read_char_delim_str(line_vec[7],'|')[3]].size()==1){
								str_vec_pair=discover_unique_variant_1pop(pop,param,var,ann,line_vec,pop_vec,row_mark,ind_vec);
							}
		...
		pair<vector<string>,vector<string> > discover_unique_variant_1pop(pop_data pop,input_param param,var_list& var,ann_data ann,vector<string> line_vec,vector<string> pop_vec,int row_mark,vector<int>& ind_vec){
			int ind1=1,ind2=1,ind3=1,ind4=1,ind5=1,valid_ind1=0,valid_ind2=0;
			...
			if(pop1_num==1&&pop_neither_num==0){
				string hline_str=generate_line_header(line_vec);
				op_str=hline_str;
				for (int i = 0; i < pop_vec.size(); ++i){
					op_str+='\t'+freq_dict[pop_vec[i]];
		...
		void multi_thread_freq_analysis(int t){
			...
			}else if(param.analysis_mode==5||param.analysis_mode==6){
				thread t1(&thread_analysis_module::ed_variant_reader,this,0,pop,param,ref(var),ann);
				for (int i = 1; i < t; ++i){
